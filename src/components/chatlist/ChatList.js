    ...
    const filterFun=(arr,searchInp)=>{
        ...
        setFilteredChats(arr.filter((ele)=>
               {   
                   ...
                   else if(umailExtractor(ele.data.membersMail[0]).includes(searchInp) && ele.data.type==='personal' && ele.data.membersMail[0]!==user0.umail) return true
                   else if(umailExtractor(ele.data.membersMail[1]).includes(searchInp) && ele.data.type==='personal' && ele.data.membersMail[1]!==user0.umail) return true
                   else return false
             })
