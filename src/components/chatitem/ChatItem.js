...
function ChatItem({uid,chatid,chatname,dp,type,members,description,memebersMail,archieved=false,blocked=false}) {
    ...
    <div>
        ...
        {type==='personal' ? 
                <div className={ false ? "chat__item chat__item__selected":"chat__item"} >
                    ...
                    <div className="time">{format(new Date('Thu Jun 17 2021 22:28:41 GMT+0530 (India Standard Time)'), ' hh:mm aaa')}</div>
                    ...
                    { options &&
                        <div className="options theme__green__bg" onClick={()=>setOptions(!options)}>
                            <div className="option__item">{archieved ? 'Unarchieve chat':'Archieve chat'}</div>
                            <div className="option__item">{blocked ? 'Unblock chat' : 'Block chat'}</div>
